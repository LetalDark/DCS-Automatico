...
# === FUNCION COMPACTA: Auto-actualizar Radio.bat (sin error rojo) ===
function Update-RadioBat {
    ...
    try {
        Write-Host "[DEBUG] Descargando ultima version de Radio.bat" -ForegroundColor Gray
        ...
        if ($needsUpdate) {
            Write-Host "[INFO] Actualizando Radio.bat..." -ForegroundColor Yellow
        ...
        Write-Host "[ERROR] No se pudo actualizar Radio.bat: $($_.Exception.Message)" -ForegroundColor Red
    }
    finally {
